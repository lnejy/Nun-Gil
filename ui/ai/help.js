...
async function handleVocab({ targetChunk, contextText, userInput }) {
  ...
  const prompt = `
"${userInput}"라는 단어/용어를 학생이 이해할 수 있게 설명해라.
...
작성 방식:
- "쉽게 말하면 ~"으로 시작하는 핵심 정의로 문을 연다.
- 단어의 난이도에 맞춰 길이를 조절해라. 간단한 용어는 2~3문장으로 짧게, 여러 개념이 얽힌 복잡한 용어는 문단을 나눠 충분히 설명한다. 억지로 늘리지 마라.
- 마지막에 이 문서·분야에서 왜 중요한지 한 줄로 짚는다.
- "1문장:", "2문장:", 번호, 헤딩, 목록 라벨 금지. 자연스러운 문단으로.
- 굵게(**)는 핵심 용어 몇 개에만.
`.trim()
...
async function handleSimplify({ targetChunk }) {
  ...
  const prompt = `
다음 단락을, 이 분야를 처음 접하는 사람도 이해할 수 있게 쉬운 말로 다시 써줘.
...
- 긴 문장은 짧게 나누기
- 핵심 뜻은 유지, 없는 내용 더하지 말 것
- 원문의 분량에 맞춰 길이를 조절해라. 짧은 원문은 간결하게, 내용이 많으면 충분히. 억지로 늘리지 마라.
- 원문이 여러 내용을 담고 있으면 내용 단위로 문단을 나눠라(빈 줄로 구분). 원문이 짧으면 한 문단으로.
- 마크다운 강조는 핵심 단어에만. 헤딩·목록·표 금지.
`.trim()
...
- 원문이 여러 내용을 담고 있으면 내용 단위로 문단을 나눠라(빈 줄로 구분)
- 표·이모지·헤딩 금지.
`.trim()
...
규칙:
규칙:
- 'c0003' 같은 식별자나 'before/after/target/role' 같은 내부 용어를 답변에 쓰지 마라.
- "앞에서는", "이 부분은", "이어지는 내용에서는" 처럼 자연스러운 한국어로.
- 학생은 문서가 조각난 걸 모른다. 하나의 글로 보고 말하듯 설명해라.
- 다음 세 덩어리를 각각 별개 문단으로, 빈 줄로 띄워서 써라:
  (1) 이 단락이 문서에서 하는 역할 / 앞 내용과의 연결
  (2) 이 단락 자체의 핵심 내용
  (3) 뒤로 어떻게 이어지는지 + 전체 흐름에서의 의미
...
window.addEventListener('doc-changed', restoreHelpMarkers)